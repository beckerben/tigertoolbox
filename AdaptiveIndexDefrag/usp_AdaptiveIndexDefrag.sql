...
BEGIN
	BEGIN TRY		
		...
		IF @dbScope IS NOT NULL AND @sqlmajorver >= 11 and (SELECT @@Version) NOT LIKE 'Microsoft SQL Azure%'
		BEGIN
...
WHERE ids.[dbID] = ' + CAST(@dbID AS NVARCHAR(10));
				...
				SELECT @updateSQL = N'SELECT DISTINCT ' + CAST(@dbID AS NVARCHAR(10)) + ', ''' + QUOTENAME(DB_NAME(@dbID)) + ''', ss.[object_id], ss.stats_id, ' + CASE WHEN ((@sqlmajorver = 12 AND @sqlbuild >= 5000) OR @sqlmajorver > 12) THEN 'ISNULL(sp.partition_number,1),' ELSE '1,' END + '
...
'AND i.object_id = ' + CAST(@objectID AS NVARCHAR(10)) + ' AND i.index_id = ' + CAST(@indexID AS NVARCHAR(10)) + ';'
			...
			BEGIN			
				...
				IF @sqlmajorver > 14
				BEGIN
					SET @sqlcmdRI = 'SELECT @HasRI_OUT = COUNT(*) FROM sys.index_resumable_operations'
			...
			BEGIN
				...
				BEGIN
					...
					SELECT @rowmodctrSQL = N' SELECT @rowmodctr_Out = ISNULL(modification_counter,0), @rows_Out = ISNULL(rows,0), @rows_sampled_Out = ISNULL(rows_sampled,0) FROM sys.dm_db_incremental_stats_properties(@statsobjectID_In, @statsID_In) WHERE partition_number = @partitionNumber_In;'
				END
				...
				BEGIN
					...
					SELECT @rowmodctrSQL = N'SELECT @rowmodctr_Out = ISNULL(modification_counter,0), @rows_Out = ISNULL(rows,0), @rows_sampled_Out = ISNULL(rows_sampled,0) FROM sys.dm_db_stats_properties_internal(@statsobjectID_In, @statsID_In) WHERE partition_number = @partitionNumber_In;'
				END
				...
				BEGIN
					...
					SELECT @rowmodctrSQL = N'SELECT @rowmodctr_Out = ISNULL(modification_counter,0), @rows_Out = ISNULL(rows,0), @rows_sampled_Out = ISNULL(rows_sampled,0) FROM sys.dm_db_stats_properties(@statsobjectID_In, @statsID_In);'
				END
				ELSE
				BEGIN
					IF @debugMode = 1
					...
					SELECT @rowmodctrSQL = N' SELECT @rowmodctr_Out = SUM(ISNULL(rowmodctr,0)), @rows_Out = ISNULL(rowcnt,0), @rows_sampled_Out = -1 FROM sys.sysindexes WHERE id = @statsobjectID_In AND indid = @statsID_In AND rowmodctr > 0;'
